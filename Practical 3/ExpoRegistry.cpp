...
EventComponent* ExpoRegistry::createComponent(ComponentType type, const std::string& parentName) {
    EventComponent* parent = !parentName.empty() ? findComponent(parentName) : nullptr;
...
bool ExpoRegistry::removeComponent(const std::string& name) {
    ...
    if (it == lookupTable.end()) return false;
...
bool ExpoRegistry::moveComponent(const std::string& childName, const std::string& newParentName) {
    EventComponent* child = findComponent(childName);
    EventComponent* newParent = findComponent(newParentName);

    if (!child || !newParent) {
        std::cerr << "[ExpoRegistry Error] Failed to move: component(s) not found in registry.\n";
        return false;
    }
...
bool ExpoRegistry::transmitSignal(const std::string& targetName, const TechSignal& signal) {
    EventComponent* target = findComponent(targetName);
    if (!target) {
        std::cerr << "[ExpoRegistry Error] Target '" << targetName << "' not found for signal transmission.\n";
        return false;
    }
...
void ExpoRegistry::printCapacity(const std::string& componentName) const {
    ...
    if (!comp) {
        std::cout << "[ExpoRegistry] Component '" << componentName << "' not found.\n";
        return;
    }

    std::cout << "[Capacity Info] " << comp->getName() 
              << ": " << comp->getCurrentVisitors() 
              << " / " << comp->getCapacity() << " visitors.\n";
}
...
void ExpoRegistry::printTreeRecursive(const EventComponent* component, int depth) const {
    if (!component) return;
